...
class rfPosts
{
	...
	public static function newThread($title, $subtitle, $fid, $content, $picurl, $ip, $zid, $special = 0, $price = 0, $tags, $also_subscribe)
	{
		...
		dbQuery(
			"INSERT INTO zf_reply (fid, tid, content, picurl, datetime, ip, authorid, isfirstpost, price) VALUES ($fid, $lastTID, :content, :picurl, NOW(), '$ip', '$zid', 1, '$price')",
		...
		dbQuery(
			"UPDATE zf_contenttype SET threadcount = threadcount + 1, datetime = NOW(), lastaid=$zid,lasttid=$lastTID,postcount=postcount+1,lastusername=:username,lasttitle=:title 
			WHERE id = :fid",
			['username' => $username, 'title' => $title, 'fid' => $fid]
		);
		if ($also_subscribe) {
			dbQuery(
				"INSERT IGNORE INTO zf_relation SET source_zid = ?, target_zid = ?, relation_type = 4",
				[$zid, $lastTID]
			);
		}
	...
	public static function replyThread($fid, $tid, $pid, $content, $picurl, $ip, $zid, $username, $price, $threadAuthorId, $also_subscribe)
	{
		...
		dbQuery(
			"INSERT INTO zf_reply (fid, tid,parent_id, content, picurl, `datetime`, ip, authorid, isfirstpost,price) 
			VALUES (?, ?, ?, ?, ?, NOW(), ?, ?, ?, ?)",
			[$fid, $tid, $pid, $content, $picurl, $ip, $zid, 0, $price]
		);
		dbQuery("UPDATE zf_contentpages SET commentnum = commentnum + 1, lastid=$zid,lastusername=:username, lastdatetime=NOW() where id = :tid",
